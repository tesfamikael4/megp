...
export class AddUserReference1714131709412 implements MigrationInterface {
  name = 'AddUserReference1714131709412';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE "procurement_requisitions" ("tenantId" integer NOT NULL DEFAULT '0', "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), "deletedAt" TIMESTAMP, "organizationId" uuid NOT NULL, "organizationName" character varying NOT NULL DEFAULT 'DefaultOrganization', "id" uuid NOT NULL DEFAULT uuid_generate_v4(), "name" character varying NOT NULL, "description" character varying, "procurementReference" character varying NOT NULL, "userReference" character varying NOT NULL, "isUsed" boolean NOT NULL DEFAULT false, "totalEstimatedAmount" numeric(14,2) NOT NULL DEFAULT '0', "calculatedAmount" numeric(14,2) NOT NULL DEFAULT '0', "currency" character varying NOT NULL DEFAULT 'USD', "procurementApplication" "public"."procurement_requisitions_procurementapplication_enum" NOT NULL DEFAULT 'tendering', "budgetId" uuid, "budgetYearId" uuid, "postBudgetPlanId" uuid, "isPlanned" boolean NOT NULL DEFAULT false, "isMultiYear" boolean NOT NULL DEFAULT false, "isFundAvailable" boolean NOT NULL DEFAULT false, "isCustom" boolean NOT NULL DEFAULT true, "remark" character varying, "status" "public"."procurement_requisitions_status_enum" NOT NULL DEFAULT 'DRAFT', CONSTRAINT "UQ_f42e85c831489da44917a4b35c7" UNIQUE ("procurementReference"), CONSTRAINT "UQ_742abcd7d3c08cdc1fd90b3384b" UNIQUE ("userReference"), CONSTRAINT "UQ_49fb9d574dbc45dd6cd704f0322" UNIQUE ("procurementReference", "deletedAt"), CONSTRAINT "PK_6d0ad196f5ce5943ac50e920255" PRIMARY KEY ("id"))`,
    );
    await queryRunner.query(
