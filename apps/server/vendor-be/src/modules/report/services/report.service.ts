...
import { BpServiceEntity, BusinessAreaEntity, InvoiceEntity, ServicePrice, VendorsEntity, WorkflowInstanceEntity } from "src/entities";
...
export class ReportService {
        ...
        for (const name of serviceNames) {
            ...
            for (const item of result) {
                ...
                if (item.service_name == name) {
                    if (item.ba_status == ApplicationStatus.OUTDATED)
                        continue;
    ...
    async getVendorsByLocation(where: any): Promise<any> {
        ...
        const result = await this.vendorRepositoty.createQueryBuilder('vendor')
            .select([
            ...
            .andWhere('ba.category In(:...categories)', { categories: ['goods', 'services', 'works'] })
    ...
    async getDashboardReport() {
        ...
        dashboard.activeVendprs = await this.countActiveVendors();
        ...
        dashboard.upgrades = await this.countUpgradesOrRenewals(ServiceKeyEnum.upgrade);
        dashboard.renewals = await this.countUpgradesOrRenewals(ServiceKeyEnum.renewal);
        const preferentials = await this.countPreferentials(ServiceKeyEnum.preferential);
        const dashbordwhole = {
            ...dashboard, ...preferentials
        }
    ...
    async countApplicationsByStatus() {
        const result = await this.baRepository.createQueryBuilder('ba')
            .select([
                'status',
                'COUNT(status) count'
            ])
            .where('ba.status In(:...statuses)', { statuses: [ApplicationStatus.APPROVED, ApplicationStatus.REJECTED, ApplicationStatus.PENDING] })
            .groupBy('ba.status')
            .getRawMany()
        const statuCount = {
            Approved: 0,
    ...
    async countPreferentials(serviceGroup: string) {
        ...
        for (const row of result) {
            ...
            if (row.key == 'Marginalized_Group')
                index = 'marginalizedGroup'
    ...
    async getRevenue(range: DateRange) {
        const result = await this.invoiceRepositoy.createQueryBuilder('invoice')
            .select([
                'invoice.serviceName name',
