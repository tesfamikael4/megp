...
export class PreferentailTreatmentService extends EntityCrudService<PreferentialTreatmentsEntity> {
  ...
  async submitPreferential(attachments: any, dto: CreatePTDto, user: any) {
    ...
    try {
      ...
      const existedRequest = await this.ptRepository.find({
        where: {
      ...
      if (dto.status == ApplicationStatus.SUBMIT) {
        ...
        const bp = await this.bpService.findBpWithServiceByServiceId(
          dto.serviceId,
        );
        ...
        if (!vendor)
          throw new BadRequestException('Please register as a vendor');
        wfi.bpId = bp.id;
