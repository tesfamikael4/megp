...
const fonturl = pathPrefix+'/assets/fonts/UcCO3FwrK3iLTeHuS_fvQtMwCp50KnMw2boKoduKmMEVuGKYMZhrib2Bg-4.ttf';
...
const VendorInformationTable = ({basicInfo}:{basicInfo:any[]}) => (
  ...
  <View style={{ padding: 15, fontSize: 11}}>
    {basicInfo.map((row, index) => (
      <View key={index} style={{color: '#333',flexDirection: 'row', alignItems: 'center', padding: 2}}>
...
const CertTable = ({businessClasses}:{businessClasses:any[]}) => (
  <View style={[pdfStyles.section]}>
    <View style={{ flexDirection: 'row', fontFamily: 'Inter', borderColor: 'gray',borderWidth: 1, borderTopLeftRadius: 4, borderTopRightRadius: 4,borderBottomWidth: 0, color: '#333'  }}>
      <Text style={[pdfStyles.cell,{borderLeftWidth: 0, borderRightWidth: 0}]}>Category</Text>
    ...
    {businessClasses.map((row, index) => (
      <View key={index} style={{ flexDirection: 'row' }}>
        <Text style={[pdfStyles.cell, {borderRightWidth: 0,    fontFamily: 'Inter', color: '#333' }]}>{row.Category}</Text>
        <Text style={[pdfStyles.cell, {borderRightWidth: 0 }]}>{row.PriceRange}</Text>
        <Text style={[pdfStyles.cell, {borderRightWidth: 0 }]}>{row.IssueDate}</Text>
