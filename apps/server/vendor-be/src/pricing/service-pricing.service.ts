...
import { ServiceKeyEnum } from 'src/handling/dtos/workflow-instance.enum';
@Injectable()
export class ServicePricingService extends EntityCrudService<ServicePrice> {
  ...
  async findServicePriceByServiceType(serviceKey: string) {
    ...
    return await this.pricingRepository.find(
      {
        select: { id: true, serviceId: true, businessArea: true, valueFrom: true, valueTo: true, fee: true, currency: true, service: { businessProcesses: { id: true }, id: true, key: true } },
        relations: { service: { businessProcesses: true } },
        where: { service: { businessProcesses: { isActive: true, }, key: In(keys) } }
