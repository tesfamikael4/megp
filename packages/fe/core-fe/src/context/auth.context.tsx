...
interface AuthContextValue {
  ...
  signUp: (formFields: SignUp) => Promise<
    ...
    | undefined
  >;
  verify: (formFields: Verify) => Promise<Record<string, never> | undefined>;
...
// const baseURL = config.ENV_AUTH_API;
const baseURL = process.env.NEXT_PUBLIC_AUTH_API;
  ...
  const buildFetchAPI = async function <T>(
    ...
    try {
      ...
      const response = await fetch(
        `${baseURL}/api/auth/${params.queryURL}`,
        fetchConfig,
      );
